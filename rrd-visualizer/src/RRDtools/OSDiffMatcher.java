...
public class OSDiffMatcher {
	...
	static private Pattern okpatLinux = Pattern
			.compile("^OK u:([0-9.]+) s:([0-9.]+) r:([0-9.]+)");
	...
	public static boolean isOKMatch(String line, CommandResult result)
		...
		if (os.indexOf("win") >= 0) {
			...
			if (linematch.find()) {
				result.user = Float.parseFloat(linematch.group(1));
				result.system = Float.parseFloat(linematch.group(2));
				result.total = Float.parseFloat(linematch.group(3));
				result.ok = true;
				whatistheresult = true;
			}
		} else
	...
	/*public static boolean isHeaderMatch(String line, CommandResult result){
		boolean whatistheresult = false;
